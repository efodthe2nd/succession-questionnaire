...
import {
  SEQUENCE_C_STEPS,
  buildSequenceCContext,
} from "@/lib/email-sequences/sequence-c";
...
async function enrollSequenceATargets() {
  ...
  for (const submission of data) {
    ...
    const hoursSince =
      (Date.now() - new Date(submission.submitted_at).getTime()) / 36e5;
...
async function processSequenceA() {
  ...
  for (const entry of pending) {
    ...
    try {
      ...
      console.error(
        `[Cron/A] Failed step ${stepConfig.step} → ${entry.email}:`,
        err,
      );
    }
  }
  ...
  for (const lead of leads) {
    const nextSendAt = new Date(
      new Date(lead.created_at).getTime() +
        SEQUENCE_B_STEPS[0].delayHours * 36e5,
    );
    await supabaseAdmin.from("email_sequence_state").insert({
      email: lead.email,
      sequence: "B",
...
const user = userList?.users?.find(u => u.email === entry.email)
...
if (user) {
  ...
  if (sub) {
    await supabaseAdmin.from('email_sequence_state')
      .update({ completed: true })
      .eq('id', entry.id)
    ...
    try {
      ...
      console.error(
        `[Cron/B] Failed step ${stepConfig.step} → ${entry.email}:`,
        err,
      );
    }
  }
...
async function enrollSequenceCTargets() {
  ...
  for (const submission of data) {
    ...
    console.log(
      `[Cron/C] Enrolled ${email} (stopped at section ${submission.current_section_index})`,
    );
  }
}
...
async function processSequenceC() {
  ...
  for (const entry of pending) {
    ...
    const ctx = buildSequenceCContext(
      stoppedAt,
      entry.email,
      entry.unsubscribe_token,
    );
    ...
    try {
      ...
      console.error(
        `[Cron/C] Failed step ${stepConfig.step} → ${entry.email}:`,
        err,
      );
    }
  }
