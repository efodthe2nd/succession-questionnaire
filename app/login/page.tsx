...
// Separate component to use useSearchParams (required by Next.js)
function LoginForm() {
  const router = useRouter();
  const searchParams = useSearchParams();
  const supabase = createClient();

  // Prefill email + detect squeeze flow
  const fromSqueeze = searchParams.get("from") === "squeeze";
    ...
    <div className="min-h-screen bg-black flex flex-col items-center justify-center px-4 relative overflow-hidden">
      ...
      <div className="relative z-10 w-full max-w-lg text-center">
        ...
        <div className="bg-gradient-to-b from-black/80 to-black/90 backdrop-blur-sm rounded-2xl p-8 pt-6 max-w-md mx-auto border border-white/10">
          {/* ── Squeeze banner ── only shows when coming from /letter */}
          {fromSqueeze && (
            <div className="flex items-start gap-3 bg-[#B5A692]/10 border border-[#B5A692]/30 rounded-lg px-4 py-3 mb-5 text-left">
              <svg
                className="w-4 h-4 text-[#B5A692] shrink-0 mt-0.5"
                fill="none"
                stroke="currentColor"
                viewBox="0 0 24 24"
              >
              ...
              <p className="text-[#B5A692] text-sm leading-relaxed">
                Check your email for your login details — then come back here to
                sign in and begin.
              </p>
            </div>
          )}
          ...
          {fromSqueeze && (
            <p className="text-[#4a4a4a] text-xs text-center mb-2">
              Can&apos;t find the email?{" "}
              <a
                href="/letter"
                className="text-[#B5A692] underline underline-offset-2"
              >
      ...
      <div className="fixed bottom-8 left-0 right-0 flex justify-center gap-8 text-white text-sm z-20">
        <a href="/terms" className="hover:text-[#B5A692] transition-colors">
          Terms
        </a>
        <a href="/privacy" className="hover:text-[#B5A692] transition-colors">
          Privacy
        </a>
      </div>
